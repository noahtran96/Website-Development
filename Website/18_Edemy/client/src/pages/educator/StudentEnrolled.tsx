...
import { STUDENT_ENROLLED } from "@/constants/StudentEnrolled";
import { useState } from "react";

export const StudentEnrolled = () => {
  const [enrolledStudents, setEnrolledStudents] = useState(STUDENT_ENROLLED);
  return enrolledStudents ? (
    <div className="flex min-h-screen flex-col items-start justify-between p-4 pt-8 pb-0 md:p-8 md:pb-0">
      <div className="flex w-full max-w-4xl flex-col items-center overflow-hidden rounded-md border border-gray-500/20 bg-white">
        <table className="w-full table-fixed overflow-hidden pb-4 md:table-auto">
          ...
          <tbody>
            {enrolledStudents.map((item, index: number) => (
